...
export default function PurchaseForm({ vehicle, onSubmit, onCancel }) {
  ...
  const submit = async (e) => {
    ...
    if (!form.buyerName || !form.buyerPhone || !form.buyerEmail || !form.buyerAddress) {
  ...
  return (
    <div className="fixed inset-0 bg-ink/60 flex items-center justify-center z-30 p-4">
      <div className="bg-paper rounded-md shadow-xl w-full max-w-md p-6 border-t-4 border-amber">
        <h2 className="font-display text-2xl uppercase mb-1">Book this vehicle</h2>
        <p className="text-sm text-steel mb-4">
          {vehicle.make} {vehicle.model} — {formatIndianCurrency(vehicle.price)}
        </p>
        ...
        <form onSubmit={submit} className="space-y-3">
          <Field label="Full name" value={form.buyerName} onChange={update("buyerName")} />
          <Field label="Phone number" value={form.buyerPhone} onChange={update("buyerPhone")} />
          <Field label="Email" type="email" value={form.buyerEmail} onChange={update("buyerEmail")} />
          <Field label="Address" value={form.buyerAddress} onChange={update("buyerAddress")} />
